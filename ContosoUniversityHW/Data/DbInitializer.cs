...
namespace ContosoUniversityHW.Data
{
	public static class DbInitializer
	{
		...
		new Instructor { FirstName = "Fadi", LastName = "Fakhouri", HireDate = DateTime.SpecifyKind(DateTime.Parse("2002-07-06"), DateTimeKind.Utc) },
		new Instructor { FirstName = "Roger", LastName = "Harui", HireDate = DateTime.SpecifyKind(DateTime.Parse("1998-07-01"), DateTimeKind.Utc) },
		new Instructor { FirstName = "Candace", LastName = "Kapoor", HireDate = DateTime.SpecifyKind(DateTime.Parse("2001-01-15"), DateTimeKind.Utc) },
		new Instructor { FirstName = "Roger", LastName = "Zheng", HireDate = DateTime.SpecifyKind(DateTime.Parse("2004-02-12"), DateTimeKind.Utc) }
			};
		...
		new Department { Name = "Mathematics", Budget = 100000, StartDate = DateTime.SpecifyKind(DateTime.Parse("2007-09-01"), DateTimeKind.Utc), InstructorID = instructors.Single(i => i.LastName == "Fakhouri").ID },
		new Department { Name = "Engineering", Budget = 350000, StartDate = DateTime.SpecifyKind(DateTime.Parse("2007-09-01"), DateTimeKind.Utc), InstructorID = instructors.Single(i => i.LastName == "Harui").ID },
		new Department { Name = "Economics", Budget = 100000, StartDate = DateTime.SpecifyKind(DateTime.Parse("2007-09-01"), DateTimeKind.Utc), InstructorID = instructors.Single(i => i.LastName == "Kapoor").ID }
			};
	...
	new Enrollment{StudentID=savedStudents[6].ID, CourseID=3141, Grade=Grade.A},
		...
		new CourseAssignment { CourseID = courses.Single(c => c.Title == "Literature").CourseID, InstructorID = instructors.Single(i => i.LastName == "Abercrombie").ID },
			...
			foreach (CourseAssignment ci in courseInstructors) context.CourseAssignments.Add(ci);
			context.SaveChanges();
		}
	}
}